    ...
    <div className="min-h-screen flex flex-col p-4">
      ...
      <main className="flex flex-1 max-w-4xl mx-auto w-full py-8">
        ...
        <aside className="w-1/4 pr-8 hidden md:block">
          ...
          <div className="mb-6 border-b border-gray-200 dark:border-gray-700 pb-4">
            ...
            {isLanguageOpen && (
              <div className="ml-2">
                {languages.map((lang) => (
                  <label key={lang.id} className="flex items-center mb-1 text-gray-700 dark:text-gray-300">
                    <input
                      type="checkbox"
                      className="form-checkbox h-4 w-4 text-blue-600 rounded mr-2"
                      checked={selectedLanguages.includes(lang.id)}
                      onChange={() => handleFilterChange('language', lang.id)}
                    />
                    {lang.name}
                  </label>
                ))}
          ...
          <div className="mb-6 pb-4">
            ...
            >
              ...
              <span>{isCompatibilityOpen ? '-' : '+'}</span>
            </button>
            {isCompatibilityOpen && (
              <div className="ml-2">
                {compatibilities.map((comp) => (
                  <label key={comp.id} className="flex items-center mb-1 text-gray-700 dark:text-gray-300">
                    <input
                      type="checkbox"
                      className="form-checkbox h-4 w-4 text-blue-600 rounded mr-2"
                      checked={selectedCompatibilities.includes(comp.id)}
                      onChange={() => handleFilterChange('compatibility', comp.id)}
                    />
                    {comp.name}
                  </label>
                ))}
        ...
        <section className="w-3/4">
          {loading && <p className="text-center text-gray-700 dark:text-gray-300">Carregando resultados...</p>}
          {error && <p className="text-center text-red-500">{error}</p>}
          {!loading && !error && searchResults.length === 0 && (
            <p className="text-center text-gray-700 dark:text-gray-300">Nenhum resultado encontrado para "{initialQuery}".</p>
          )}
          {!loading && !error && searchResults.length > 0 && (
            <>
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">
                {searchResults.length} agentes de automação encontrados para "{initialQuery}"
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                {searchResults.map((product) => (
                  <div key={product.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 border border-gray-200 dark:border-gray-700">
                    ...
                    <a
                      href={`/agents/${product.name.toLowerCase().replace(/\s+/g, '-')}`} // Example link generation
